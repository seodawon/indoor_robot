...
options = {
    ...
    published_frame = "map",    -- 최종적으로 퍼블리시할 프레임 -> 일반적으로 map_frame과 동일하게 설정
    ...
    lookup_transform_timeout_sec = 0.1,  -- 좌표 변환을 조회할 때의 타임아웃 설정  -> 서로 다른 좌표계 간의 변환을 조회할 때 일정 시간이 걸리는데 타임아웃 시간을 초과하면 변환 조회가 실패로 처리
    submap_publish_period_sec = 0.1,  -- 서브맵(부분 지도)을 '0.1초'마다 퍼블리시
    pose_publish_period_sec = 100e-3, -- 로봇의 포즈(위치와 자세) 퍼블리시 주기 
    trajectory_publish_period_sec = 100e-3, -- 경로 데이터 퍼블리시 주기 
...
TRAJECTORY_BUILDER_2D.max_range = 3.0  -- LIDAR가 인식할 수 있는 최대 거리 설정 ->  3.0미터보다 먼 거리는 무시
TRAJECTORY_BUILDER_2D.missing_data_ray_length = 3. -- 데이터가 누락된 경우 해당 광선(레이)의 길이 설정
TRAJECTORY_BUILDER_2D.use_imu_data = true -- IMU 데이터를 사용하도록 설정

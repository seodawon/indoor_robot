...
class Republisher(Node):
    def __init__(self):
        ...
        self.img_sub = self.create_subscription(Image, '/inference/image_raw', self.img_callback, 10) # 이미지 보내줌
        ...
        self.imgdata_sub = self.create_subscription(BoundingBoxArray, '/inference/object_raw', self.imgdata_callback, 10) # 이미지 근본적인 정보
        self.point_data= self.create_publisher(SensorFusion,'/camera_dataR',10)
        ...
        self.camera = np.array([[0.00],[0],[0.08]])  # 카메라 위치
        ...
        roll = np.radians(-90)          # X축 회전은 없으므로 0도
    ...
    def lidar_callback(self, msg):
        if self.img is not None :
            ...
            for i in range (111):#169 #여기 각도만큼 반복
                project_lidar_to_image(msg.ranges[645 + i], 152.5   + self.inc_angle * i, self) # 여기
                if len(self.box) > 0:
                    if self.u is not None and self.v is not None and self.box is not None:
                        catch(self, msg.ranges[645 + i], 152.5  + self.inc_angle * i, i) #여기
                # elif len(self.box) == 0:        
                #      break
                # project_lidar_to_image(msg.ranges[645 + i], 152.5  + self.inc_angle * i, self) # 여기
            Fusion(self.img)
...
def catch(self, dis, angle, count): # 박스 안에 있어? 없어?
    for i in range(len(self.box)): # 박스 개수에 따라 여러개가 된다면? 
        ...
        if x_max >= self.u >= x_min and class_name != "bracelet": #send_data에 값을 담아서 보냄 그런데 bracelet 즉 팔찌는 안보냄
            # self.send_data.append((class_name,dis))
